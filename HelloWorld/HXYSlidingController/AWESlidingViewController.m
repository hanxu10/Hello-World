...
@interface UIScrollView (AWESlidingScroll)

- (NSInteger)currentPageIndex;

@end

@implementation UIScrollView (AWESlidingScroll)
...
@implementation AWESlidingViewController
...
- (void)setTabbarView:(UIView<AWESlidingTabbarProtocol> *)tabbarView
{
...
- (void)updateSelectedIndexWithScrollView:(UIScrollView *)scrollView
{
    ...
    [self updateScrollIndexControllerWithTransitionType:AWESlidingVCTransitionTypeScroll index:index];
}
...
- (void)scrollToIndex:(NSInteger)index animated:(BOOL)animated
{
    ...
    [self updateScrollIndexControllerWithTransitionType:AWESlidingVCTransitionTypeTapTab index:index];
    [self.contentScrollView setContentOffset:CGPointMake(targetOffset, 0) animated:animated];
}

- (void)updateScrollIndexControllerWithTransitionType:(AWESlidingVCTransitionType)transitionType index:(NSInteger)index
{
    ...
    if (self.tabbarView) {
        if ([self.tabbarView respondsToSelector:@selector(setSelectedIndex:animated:tapped:)]) {
            BOOL tapped = (transitionType == AWESlidingVCTransitionTypeTapTab);
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
    ...
    if (nextIndex >= 0 && nextIndex < self.viewControllers.count) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(slidingViewController:willTransitionToViewController:transitionType:)]) {
            [self.delegate slidingViewController:self willTransitionToViewController:[self currentVCAtIndex:nextIndex] transitionType:AWESlidingVCTransitionTypeScroll];
        }
    }
}
...
- (void)_makeViewControllerVisibleAtIndex:(NSInteger)index transitionType:(AWESlidingVCTransitionType)transitionType
{
    if (index >= self.viewControllers.count || index < 0) {
        return;
    }
    if (transitionType == AWESlidingVCTransitionTypeScroll) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(slidingViewController:willTransitionToViewController:transitionType:)]) {
            [self.delegate slidingViewController:self willTransitionToViewController:[self currentVCAtIndex:index] transitionType:AWESlidingVCTransitionTypeTapTab];
        }
        [self _makeControllersDidAppearAtPreIndex:self.selectedIndex currentIndex:index];
    }
...
{
    return [AWESlidingScrollView class];
}
